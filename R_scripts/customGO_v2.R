...
customGO <- function(DE, DE_upregulated, DE_downregulated, title, subtitle = "") {
  ...
  all_dotplot <- dotplot(ego_custom) + 
    ggtitle(paste0(title, " - Custom Modules")) +
  ...
  up_dotplot <- dotplot(ego_custom_up) + 
    ggtitle(paste0("Upregulated in ", title, " - Custom Modules")) +
    labs(subtitle = subtitle)
  ...
  down_dotplot <- dotplot(ego_custom_down) + 
    ggtitle(paste0("Downregulated in ", title, " - Custom Modules")) +
    labs(subtitle = subtitle)
...
# do custom gene set analysis for DE_DKO_adj
load("downstream_inputs/DE_DKO_adj_inputs.RData")
DE_DKO_adj_customGO <- customGO(DE_DKO_adj, DE_DKO_adj_upregulated, DE_DKO_adj_downregulated, "DKO Case vs Control", "Adjusted with respect to WT case vs control")
all_DE_DKO_adj_dotplot_custom <- DE_DKO_adj_customGO$all_dotplot
up_DE_DKO_adj_dotplot_custom <- DE_DKO_adj_customGO$up_dotplot
down_DE_DKO_adj_dotplot_custom <- DE_DKO_adj_customGO$down_dotplot

# do custom gene set analysis for DE_healthy
load("downstream_inputs/DE_healthy_inputs.RData")
DE_healthy_customGO <- customGO(DE_healthy, DE_healthy_upregulated, DE_healthy_downregulated, "DKO vs WT in Healthy Control")
all_DE_healthy_dotplot_custom <- DE_healthy_customGO$all_dotplot
up_DE_healthy_dotplot_custom <- DE_healthy_customGO$up_dotplot
down_DE_healthy_dotplot_custom <- DE_healthy_customGO$down_dotplot
...
print(all_DE_DKO_adj_dotplot_custom)
print(up_DE_DKO_adj_dotplot_custom)
print(down_DE_DKO_adj_dotplot_custom)

print(all_DE_healthy_dotplot_custom)
print(up_DE_healthy_dotplot_custom)
print(down_DE_healthy_dotplot_custom)
...
DKO_adj_tophits <- DE_DKO_adj_renamed[order(DE_DKO_adj_renamed$padj),]
